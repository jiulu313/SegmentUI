...
import com.segment.ui.core.Segment;
import com.segment.ui.util.LayoutHelper;
...
public class ActionBarLayout extends FrameLayout {
    public interface ActionBarLayoutDelegate {
        ...
        boolean needPresentFragment(Segment segment, boolean removeLast, boolean forceWithoutAnimation, ActionBarLayout layout);

        boolean needAddFragmentToStack(Segment segment, ActionBarLayout layout);
    ...
    public class LinearLayoutContainer extends LinearLayout {
        ...
        protected void onLayout(boolean changed, int l, int t, int r, int b) {
            ...
            int usableViewHeight = rootView.getHeight() - (rect.top != 0 ? SAppUtil.statusBarHeight : 0) - AndroidUtilities.getViewInset(rootView);
    ...
    public ArrayList<Segment> fragmentsStack = null;
    ...
    public void init(ArrayList<Segment> stack) {
        ...
        for (Segment Segment : fragmentsStack) {
            Segment.setParentLayout(this);
        }
    }
    ...
    public void onConfigurationChanged(android.content.res.Configuration newConfig) {
        ...
        if (!fragmentsStack.isEmpty()) {
            Segment lastFragment = fragmentsStack.get(fragmentsStack.size() - 1);
    ...
    private void onSlideAnimationEnd(final boolean backAnimation) {
        if (!backAnimation) {
            ...
            Segment lastFragment = fragmentsStack.get(fragmentsStack.size() - 2);
            lastFragment.onPause();
        ...
        if (!checkTransitionAnimation() && !inActionMode && !animationInProgress) {
            if (fragmentsStack.size() > 1) {
                if (ev != null && ev.getAction() == MotionEvent.ACTION_DOWN && !startedTracking && !maybeStartTracking) {
                    Segment currentFragment = fragmentsStack.get(fragmentsStack.size() - 1);
    ...
    private void presentFragmentInternalRemoveOld(boolean removeLast, final Segment Segment) {
        if (Segment == null) {
            return;
        }
        Segment.onPause();
        if (removeLast) {
            Segment.onFragmentDestroy();
            Segment.setParentLayout(null);
            fragmentsStack.remove(Segment);
        } else {
            if (Segment.fragmentView != null) {
                ViewGroup parent = (ViewGroup) Segment.fragmentView.getParent();
                if (parent != null) {
                    Segment.onRemoveFromParent();
                    parent.removeView(Segment.fragmentView);
                }
            }
            if (Segment.actionBar != null && Segment.actionBar.getAddToContainer()) {
                ViewGroup parent = (ViewGroup) Segment.actionBar.getParent();
                if (parent != null) {
                    parent.removeView(Segment.actionBar);
                }
            }
        }
        containerViewBack.setVisibility(View.GONE);
    ...
    public boolean presentFragment(Segment Segment) {
        return presentFragment(Segment, false, false, true);
    }

    public boolean presentFragment(Segment Segment, boolean removeLast) {
        return presentFragment(Segment, removeLast, false, true);
    }
    ...
    public boolean presentFragment(final Segment Segment, final boolean removeLast, boolean forceWithoutAnimation, boolean check) {
        if (checkTransitionAnimation() || delegate != null && check && !delegate.needPresentFragment(Segment, removeLast, forceWithoutAnimation, this) || !Segment.onFragmentCreate()) {
            return false;
        }
        if (parentActivity.getCurrentFocus() != null) {
            AndroidUtilities.hideKeyboard(parentActivity.getCurrentFocus());
        ...
        final Segment currentFragment = !fragmentsStack.isEmpty() ? fragmentsStack.get(fragmentsStack.size() - 1) : null;

        Segment.setParentLayout(this);
        View fragmentView = Segment.fragmentView;
        if (fragmentView == null) {
            fragmentView = Segment.createView(parentActivity);
        } else {
            ViewGroup parent = (ViewGroup) fragmentView.getParent();
            if (parent != null) {
                Segment.onRemoveFromParent();
        ...
        if (Segment.actionBar != null && Segment.actionBar.getAddToContainer()) {
            if (removeActionBarExtraHeight) {
                Segment.actionBar.setOccupyStatusBar(false);
            }
            ViewGroup parent = (ViewGroup) Segment.actionBar.getParent();
            if (parent != null) {
                parent.removeView(Segment.actionBar);
            }
            containerViewBack.addView(Segment.actionBar);
            Segment.actionBar.setTitleOverlayText(titleOverlayText, subtitleOverlayText, overlayAction);
        }
        ...
        layoutParams.height = LayoutHelper.MATCH_PARENT;
        fragmentView.setLayoutParams(layoutParams);
        fragmentsStack.add(Segment);
        Segment.onResume();
        currentActionBar = Segment.actionBar;
        if (!Segment.hasOwnBackground && fragmentView.getBackground() == null) {
            fragmentView.setBackgroundColor(Theme.getColor(Theme.key_windowBackgroundWhite));
        }
        ...
        if (needAnimation) {
            if (useAlphaAnimations && fragmentsStack.size() == 1) {
                ...
                onOpenAnimationEndRunnable = new Runnable() {
                    @Override
                    public void run() {
                        Segment.onTransitionAnimationEnd(true, false);
                        Segment.onBecomeFullyVisible();
                    }
                };
                ...
                Segment.onTransitionAnimationStart(true, false);
                ...
                Segment.onTransitionAnimationStart(true, false);
                AnimatorSet animation = Segment.onCustomTransitionAnimation(true, new Runnable() {
                    @Override
                    public void run() {
                        onAnimationEndCheck(false);
                    }
                ...
                if (animation == null) {
                    ...
                    } else if (Segment.needDelayOpenAnimation()) {
    ...
    public boolean addFragmentToStack(Segment Segment) {
        return addFragmentToStack(Segment, -1);
    }

    public boolean addFragmentToStack(Segment Segment, int position) {
        if (delegate != null && !delegate.needAddFragmentToStack(Segment, this) || !Segment.onFragmentCreate()) {
            return false;
        }
        Segment.setParentLayout(this);
        if (position == -1) {
            if (!fragmentsStack.isEmpty()) {
                Segment previousFragment = fragmentsStack.get(fragmentsStack.size() - 1);
            ...
            fragmentsStack.add(position, Segment);
        }
        return true;
    }

    private void closeLastFragmentInternalRemoveOld(Segment Segment) {
        Segment.onPause();
        Segment.onFragmentDestroy();
        Segment.setParentLayout(null);
        fragmentsStack.remove(Segment);
    ...
    public void closeLastFragment(boolean animated) {
        ...
        final Segment currentFragment = fragmentsStack.get(fragmentsStack.size() - 1);
        Segment previousFragment = null;
        if (fragmentsStack.size() > 1) {
        ...
        if (previousFragment != null) {
            ...
            if (needAnimation) {
                transitionAnimationStartTime = System.currentTimeMillis();
                transitionAnimationInProgress = true;
                final Segment previousFragmentFinal = previousFragment;
    ...
    public void showLastFragment() {
        ...
        for (int a = 0; a < fragmentsStack.size() - 1; a++) {
            Segment previousFragment = fragmentsStack.get(a);
            if (previousFragment.actionBar != null && previousFragment.actionBar.getAddToContainer()) {
                ViewGroup parent = (ViewGroup) previousFragment.actionBar.getParent();
                if (parent != null) {
                    parent.removeView(previousFragment.actionBar);
    ...
    private void removeFragmentFromStackInternal(Segment Segment) {
        Segment.onPause();
        Segment.onFragmentDestroy();
        Segment.setParentLayout(null);
        fragmentsStack.remove(Segment);
    ...
    public void removeFragmentFromStack(Segment Segment) {
        if (useAlphaAnimations && fragmentsStack.size() == 1 && AndroidUtilities.isTablet()) {
            closeLastFragment(true);
        } else {
            removeFragmentFromStackInternal(Segment);
        }
    }
